...
import {
  CheckCircle2, XCircle, UserPlus, UserMinus, RefreshCw, Ban, Tag, Trophy, Bell, MessageCircle, CheckCheck,
} from 'lucide-react';
...
// PRD §7.8 — the eight example notification types
      ...
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {unread > 0 ? `${unread} unread` : 'All caught up'} · WhatsApp-delivered messages are mirrored here.
          </p>
        </div>
        {unread > 0 && (
            ...
            {mine.map((n) => {
              ...
              return (
                <button
                  ...
                  className={cn('flex w-full items-start gap-3 p-4 text-left transition-colors hover:bg-muted/50', !n.isRead && 'bg-primary/[0.03]')}
                >
                  <span className={cn('mt-0.5 flex size-9 shrink-0 items-center justify-center rounded-xl', n.isRead ? 'bg-muted text-muted-foreground' : 'bg-primary/10 text-primary')}>
